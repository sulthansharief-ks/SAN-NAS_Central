...
**Context:** Comprehensive guide covering deployment, component interconnection (Probes), master exhaustive task lists, daily operations, advanced system administration, log collection, and troubleshooting.  
...
* [3. System Administration, Maintenance & Logging ⚙️](#3-system-administration)
    ...
    * [3.2 Troubleshooting: Storage System Not Visible](#32-storage-system-not-visible)
    ...
    * [3.6 Log Collection (Dump Files & Audit Logs)](#36-log-collection)
    * [3.7 License Management](#37-license-management)
* [4. Daily Operations (Provisioning, Reclaiming & Mobility) 🗄️](#4-daily-operations)
    * [4.1 Registering a New Host (Server)](#41-registering-host)
    * [4.2 Allocating & Mapping Volumes](#42-allocating-volumes)
    * [4.3 Detaching (Unmapping) Volumes](#43-detaching-volumes)
    * [4.4 Deleting / Shredding Volumes](#44-deleting-volumes)
...
* **Administrator (Rainier):** The execution engine. Pushes REST API commands directly to the array's SVP or GUM to carve storage and map LUNs.
...
* **Protector:** The copy-data management tool. Orchestrates local snapshots (Thin Image) and remote replication (TrueCopy / GAD).
...
* **What it is:** A lightweight data-collector server (often a separate VM).
...
* **The Flow:** Endpoints -> Analyzer Probe -> Ops Center Analyzer Server. This separates the heavy lifting of data collection from the AI analysis engine.
...
4. *Warning:* If Ops Center polled the array with an expired password too many times, the local user account on the VSP may be locked. You must log directly into the array's SVP/GUM to unlock the account first.
...
2. Click the gear icon (Settings) in the top right > **Security**.
...
5. Assign granular roles (e.g., Storage Administrator, Security Administrator, Monitor).

### <a id="35-registering-analyzer-probe"></a>**3.5 Registering an Analyzer Probe to Ops Center 📡**
**Step A: Register the Probe Server**
1. Log in to **Ops Center Analyzer**.
2. Click the **Gear Icon** > **Data Collection**.
...
2. Click **Add Target**.
3. Select the resource type (e.g., Hitachi VSP System, VMware vCenter).
...
**A. Collecting Array Dump Files (Trace Logs)**
1. Log in to **Ops Center Administrator**.
...
**C. Downloading Audit Logs**
1. In Ops Center Administrator, click the **Gear Icon** (Settings) > **Audit Logs**.
...
3. Enter the **Server Name** (e.g., PROD_ESXi_01).
...
3. Scroll down to the **Attached Volumes** tab. Check the box next to the volume(s) you wish to remove.
4. Click the **Detach** button. Confirm the action.
...
3. Check the box next to the volume. Click the Trash Can icon (**Delete**).
4. **Shredding (Optional):** Check the box for **Shred Volume** during deletion. Ops Center will overwrite the volume with zeros or a 3-pass DoD pattern.
5. Click **Submit**.

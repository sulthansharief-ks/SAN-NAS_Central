...
## 📋 Phase 1: Comprehensive Pre-Upgrade Checks
...
3.  **Verify Host Multipathing (MPIO):** 🛣️
    * **How:** Instruct server admins to run `multipath -ll` (Linux) or check the MPIO control panel (Windows) / vCenter Storage Paths (ESXi).
...
4.  **Continuous Ping Monitoring:** 📡
    ...
    * **Action:** Start a continuous ping to both Controller 1 and Controller 2 management IPs (`ping -t <IP>` on Windows, `ping <IP>` on Linux). Leave this running for the entire upgrade.

---

## 📤 Phase 2: Firmware Upload & Preparation
...
## 🔀 Phase 3: The Upgrade Execution (Standard Path)
...
## ⚠️ Phase 3B: The Upgrade Execution (HDD Warning Exception Path)
